...
@tool
async def get_all_doctors() -> str:
    """Récupère la liste de tous les médecins enregistrés dans le système."""
    try:
        result = await call_business_service("doctor-service", "/api/doctors")
        return str(result)
    except Exception as e:
        return f"Erreur lors de la récupération de la liste des médecins : {e}"
